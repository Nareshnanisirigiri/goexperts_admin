    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
          onClick={openAdd}
          className="bg-[#F24C20] hover:bg-[#d43a12] text-white px-8 py-3 rounded-xl font-medium flex items-center justify-center gap-2">
                ...
                {loading ? (
                  ...
                  <motion.tr key={cat._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                    ...
                    <td className="px-6 py-4">
                      <div className="relative w-10 h-10 flex items-center justify-center bg-gray-50 dark:bg-gray-900 rounded-lg overflow-hidden border border-gray-100 dark:border-gray-800">
                        {/* Fallback Icon */}
                    ...
                    <td className="px-6 py-4 text-sm">
                      {cat.parent ? (
                        <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 whitespace-nowrap">
                          {cat.parent.name}
                        </span>
                      ) : (
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 max-w-xs">
                      <div className="line-clamp-2" title={cat.description || ''}>
                        {cat.description || '—'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleToggle(cat)} className={`p-2 rounded-lg transition-colors ${cat.is_active ? 'hover:bg-green-100 dark:hover:bg-green-900/20' : 'bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600'}`} title="Toggle Status">
                          {cat.is_active ? <ToggleRight className="w-4 h-4 text-green-600" /> : <ToggleLeft className="w-4 h-4 text-white" />}
                        </motion.button>
