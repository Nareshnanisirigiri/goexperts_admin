...
interface ProjectsListProps {
  ...
  initialFilter?: 'all' | 'in_queue' | 'approved' | 'rejected' | 'flagged';
}

export function ProjectsList({ onSelectProject, initialFilter = 'all' }: ProjectsListProps) {
  const [projects, setProjects] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [filterStatus, setFilterStatus] = useState<'all' | 'in_queue' | 'approved' | 'rejected' | 'flagged'>(initialFilter);
                ...
                {paginatedProjects.map((project: any, index: number) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                      {(() => {
                        const dateVal = project.created_at || project.createdAt || project.createdDate;
