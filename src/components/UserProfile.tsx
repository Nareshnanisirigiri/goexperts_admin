...
export function UserProfile({ userId, onBack }: UserProfileProps) {
  ...
  return (
    <div className="min-h-screen theme-bg-gradient p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <motion.div
          ...
          className="theme-card rounded-2xl border overflow-hidden shadow-sm"
        >
            ...
            <div className="lg:w-[200px] flex-shrink-0">
              <div className="relative">
                ...
                <img
                  src={profileImage}
                  alt={user.full_name}
                  className="relative w-[200px] h-[200px] rounded-2xl object-cover border-4 theme-card"
            ...
            <div className="flex-1 space-y-4">
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-3">
                  <h1 className="text-3xl font-black theme-text-primary tracking-tight">{user.full_name}</h1>
                  {user.role_title && (
                ...
                <div className="flex flex-wrap gap-2">
                  {user.is_email_verified && (
                    <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded border uppercase tracking-wider pill-success">
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl border theme-button">
                  <Star className="w-4 h-4 text-amber-500" />
                  <span className="theme-text-primary font-bold">{rating}</span>
                  <span className="theme-text-muted text-xs">({user.review_count || 0} reviews)</span>
                </div>
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl border theme-button">
                  <Award className="w-4 h-4 text-blue-500" />
                  <span className="theme-text-primary font-bold">₹{user.hourly_rate || 0}</span>
                  <span className="theme-text-muted text-xs">/service</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className={`flex-1 min-w-[140px] px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all duration-200 flex items-center justify-center gap-2 border shadow-md ${!user.is_suspended
              ? 'btn-danger-theme'
              : 'btn-success-theme'
              }`}
          >
          ...
          <button
            ...
            className="flex-1 min-w-[140px] px-4 py-3 border rounded-xl font-bold text-xs uppercase tracking-widest transition-all duration-200 flex items-center justify-center gap-2 shadow-md theme-button text-red-500 hover:bg-red-600 hover:text-white"
          >
            <X className="w-4 h-4" />
        ...
        {/* Tabs Navigation */}
        <div className="flex gap-2 border-b border-gray-200 theme-card">
          <button
          ...
          {activeTab === 'overview' && (
            ...
            >
              ...
              <div className="theme-card rounded-2xl border p-6 shadow-sm">
                <h3 className="text-xs font-bold theme-text-muted uppercase tracking-widest flex items-center gap-2 mb-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="theme-card rounded-2xl border p-6 shadow-sm">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between py-2 border-b theme-card">
                      <span className="theme-text-secondary text-sm">Role</span>
                      <span className="theme-text-primary text-sm font-medium">{primaryRole}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b theme-card">
                      <span className="theme-text-secondary text-sm">Location</span>
                      <span className="theme-text-primary text-sm">{user.location || '—'}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b theme-card">
                      <span className="theme-text-secondary text-sm">WhatsApp</span>
                      <span className="theme-text-primary text-sm">
                        {`${user.whatsapp_country_code || ''} ${user.whatsapp_number || ''}`.trim() || '—'}
                      </span>
                    </div>
                    <div className="flex justify-between py-2 border-b theme-card">
                      <span className="theme-text-secondary text-sm">Alternative Number</span>
                      <span className="theme-text-primary text-sm">
                  ...
                  <div className="mb-4">
                    <div className="flex flex-wrap gap-2">
                      {user.categories?.length ? user.categories.map((c: any, i: number) => (
                        <span key={i} className="px-3 py-1.5 text-xs rounded-lg font-medium border theme-pill">
                          {typeof c === 'object' ? c.name : c}
                        </span>
                      )) : <span className="theme-text-muted text-sm italic">No categories selected</span>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* SEO Metadata */}
              <div className="theme-card rounded-2xl border p-6 shadow-sm">
                <h3 className="text-xs font-bold theme-text-muted uppercase tracking-widest mb-4 flex items-center gap-2">
                  <Globe className="w-4 h-4 text-blue-500" />
                  SEO & Metadata
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-bold theme-text-secondary uppercase mb-1 block">Meta Title</label>
                    <p className="text-sm theme-text-primary font-medium">{user.meta_title || '—'}</p>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold theme-text-secondary uppercase mb-1 block">Meta Keywords</label>
                    <p className="text-sm theme-text-secondary">{user.meta_keywords || '—'}</p>
                  </div>
                  <div className="md:col-span-2">
                    <label className="text-[10px] font-bold theme-text-secondary uppercase mb-1 block">Meta Description</label>
                    <p className="text-sm theme-text-secondary leading-relaxed">{user.meta_description || '—'}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {user.social_links.linkedin && (
                      <a href={user.social_links.linkedin} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-lg border transition-all theme-button hover:bg-[#F24C20] hover:text-white">
              ...
              {user.portfolio?.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {user.portfolio.map((project: any, idx: number) => (
                    <div key={idx} className="rounded-xl overflow-hidden border hover:border-[#F24C20]/30 transition-all theme-card">
                      {project.image && (
                        <img
                      ...
                      <div className="p-4">
                        <h4 className="theme-text-primary font-medium mb-1">{project.title}</h4>
                        <p className="theme-text-secondary text-xs line-clamp-2 mb-3">{project.description}</p>
                ...
                <p className="theme-text-muted text-center py-8">No portfolio items available</p>
              )}
            </motion.div>
          )}
            ...
            >
              ...
              <div className="space-y-3">
                ...
                {user.documents?.educational?.length > 0 && (
                  <div className="pt-3">
                    <p className="theme-text-secondary text-xs mb-2">Educational Documents</p>
                    <div className="flex gap-2">
                      {user.documents.educational.map((doc: string, idx: number) => (
                        <button
                          ...
                          className="p-2 rounded-lg transition-all theme-button"
                        >
        ...
        {previewDocument && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-4xl max-h-[85vh] rounded-2xl border overflow-hidden flex flex-col shadow-2xl theme-card"
            >
              <div className="flex items-center justify-between p-4 border-b bg-white theme-card">
                <h3 className="font-medium theme-text-primary">{previewDocument.title}</h3>
                <div className="flex gap-2">
                  <a
                    href={previewDocument.url}
                    download
                    className="p-2 rounded-lg border transition-all theme-button"
                  >
...
function DocumentItem({ label, exists, onClick }: { label: string; exists: boolean; onClick: () => void }) {
  return (
    <div className="flex items-center justify-between p-3 rounded-xl border theme-button">
      <div className="flex items-center gap-3">
        <FileText className={`w-4 h-4 ${exists ? 'text-emerald-500' : 'theme-text-muted'}`} />
        <span className="text-sm theme-text-primary">{label}</span>
      </div>
      {exists ? (
        <button onClick={onClick} className="p-1.5 rounded-lg border transition-all theme-button hover:bg-[#F24C20] hover:text-white">
          <Eye className="w-3.5 h-3.5" />
        </button>
      ) : (
        <span className="text-xs theme-text-muted">Not uploaded</span>
      )}
    </div>
  );
}
