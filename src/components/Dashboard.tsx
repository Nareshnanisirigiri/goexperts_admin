...
export function Dashboard({ onNavigate, onSelectUser, onSelectProject, onSelectDispute, onSelectOrder }: DashboardProps) {
  ...
  const handleActivityClick = (activity: any) => {
    switch (activity.type) {
      case 'user_signup':
        onSelectUser?.('8'); // Alex Thompson ID
        break;
      case 'project_posted':
        onSelectProject?.('13'); // Wireless internet project ID
        break;
      case 'gig_purchased':
        onSelectOrder?.('GO004'); // school project order ID
        break;
      case 'withdraw_requested':
        onNavigate?.('withdraw-requests');
        break;
      case 'dispute_raised':
        onSelectDispute?.('DS001'); // Incomplete delivery dispute ID
        break;
      case 'user_verified':
        onSelectUser?.('3'); // Emma Rodriguez ID
        break;
      case 'gig_approved':
        onNavigate?.('projects');
        break;
      case 'project_completed':
        onSelectProject?.('15'); // WordPress website pages project ID
        break;
      default:
        break;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <button 
            onClick={(e) => { e.stopPropagation(); onNavigate?.('users-new'); }}
            className="text-sm text-[#F24C20] hover:text-[#d43a12] font-medium text-left"
          >
            View Details →
          </button>
        </motion.div>
        ...
        >
          ...
          <button 
            onClick={(e) => { e.stopPropagation(); onNavigate?.('disputes'); }}
            className="text-sm text-[#F24C20] hover:text-[#d43a12] font-medium text-left"
          >
            Resolve →
          </button>
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {[
            { label: 'Approve Projects', primary: true, page: 'pending-approvals' },
            { label: 'Verify Users', primary: false, page: 'verification' },
            { label: 'Review Withdraws', primary: false, page: 'withdraw-requests' },
            { label: 'Resolve Disputes', primary: true, page: 'disputes' },
            { label: 'Post Banner', primary: false, page: 'banners' },
            { label: 'Create Category', primary: false, page: 'categories' }
          ].map((action, index) => (
            <motion.button
              key={action.label}
